...
const FRONT_CHAIN = [0, 11, 12, 23, 24, 25, 26, 27, 28];
const LEFT_CHAIN = [11, 13, 15, 19, 21, 23, 25, 27, 29, 31];
const RIGHT_CHAIN = [12, 14, 16, 20, 22, 24, 26, 28, 30, 32];
const FMS_TUNING = {
    deepSquat: {
        startKneeDropDeg: 18,
...
function landmarkIsConfident(landmark) {
    if (!landmark)
        return false;
    return confidenceOf(landmark) >= MIN_LANDMARK_CONFIDENCE;
}
function allLandmarksPresent(landmarks, indices) {
...
function segmentAngleToHorizontalDeg(a, b) {
    return segmentAngleDeg(a, b);
}
function verticalityErrorDeg(a, b) {
...
function averageConfidence(landmarks, indices) {
    return mean(indices.map((index) => confidenceOf(landmarks[index] ?? { x: 0, y: 0 })));
}
function selectVisibleSide(landmarks) {
...
function toBodyScale(landmarks) {
...
function minValue(...values) {
    return Math.min(...values.filter((value) => typeof value === 'number'));
}
function maxValue(...values) {
    return Math.max(...values.filter((value) => typeof value === 'number'));
}
function formatMetric(label, value, unit = '') {
    const rounded = Math.round(value * 10) / 10;
    return `${label}: ${rounded}${unit}`;
}
function shoulderMobilityHandLength(landmarks, side) {
    const indices = sideIndices(side);
    return maxValue(distance(landmarks[indices.wrist], landmarks[indices.index]), distance(landmarks[indices.wrist], landmarks[indices.thumb]));
}
function handPlacementClassForPushUp(landmarks, side, sex) {
    ...
    if (sex === 'female') {
        if (closest === chinDistance)
            return 3;
        if (closest === clavicleDistance)
            return 2;
        return 1;
    }
    if (closest === foreheadDistance)
        return 3;
    if (closest === chinDistance)
        return 2;
    return 1;
}
function requiredIndicesForTask(task) {
...
function defaultAssessment(phase, lockFrames, guidance, landmarks) {
    const statusLabel = phase === 'awaiting_pose'
        ? 'Center your body'
        : phase === 'locking'
...
export class FmsTaskAnalyzer {
    ...
    constructor(task, sex, mode = 'primary') {
        this.task = task;
        this.sex = sex;
        this.mode = mode;
    }
    get hasAnnouncedReady() {
    ...
    processFrame(frame, timestampMs) {
        ...
        if (this.phase === 'awaiting_pose' || this.phase === 'locking') {
            ...
            return defaultAssessment(this.phase, Math.min(this.lockFrames, FMS_LOCK_FRAME_TARGET), this.phase === 'ready' ? 'Perform the movement now.' : 'Hold still while the app verifies your setup.', frame.landmarks);
        }
        if (!this.baseline) {
            this.baseline = this.createBaseline(sample, frame.landmarks);
        }
    ...
    createBaseline(sample, landmarks) {
        return {
            side: sample.side,
    ...
    buildSample(landmarks, worldLandmarks, timestampMs) {
        if (!allLandmarksPresent(landmarks, FRONT_CHAIN))
            return null;
        ...
        switch (this.task.patternKey) {
            ...
            case 'shoulderMobility': {
                ...
                const handLengthNorm = normalize((shoulderMobilityHandLength(landmarks, topSide) + shoulderMobilityHandLength(landmarks, bottomSide)) / 2, bodyScale);
            ...
            case 'trunkStabilityPushUp': {
                ...
                const bodyLineErrorDeg = angleDifferenceDeg(segmentAngleDeg(landmarks[forward.shoulder], landmarks[forward.hip]), segmentAngleDeg(landmarks[forward.hip], landmarks[forward.ankle]));
                const hipToLineY = landmarks[forward.hip].y -
            ...
            case 'rotaryStability': {
                ...
                return {
                    ...
                    trunkRotationDeg: worldLandmarks.length >= 25
    ...
    didMovementStart(sample, baseline) {
        switch (this.task.patternKey) {
            case 'deepSquat':
                return ((baseline.kneeAngleDeg - (sample.kneeAngleDeg ?? baseline.kneeAngleDeg)) >= FMS_TUNING.deepSquat.startKneeDropDeg ||
            ...
            case 'trunkStabilityPushUp':
                return (normalize((baseline.shoulderLiftY ?? sample.shoulderLiftY ?? 0) - (sample.shoulderLiftY ?? 0), baseline.bodyScale) >= FMS_TUNING.trunkStabilityPushUp.startLiftNorm ||
            ...
            case 'rotaryStability':
                return (normalize((sample.movingWristX ?? 0) - (baseline.movingWristX ?? sample.movingWristX ?? 0), baseline.bodyScale) >= FMS_TUNING.rotaryStability.extensionStartNorm ||
    ...
    createCaptureAccumulator(sample, timestampMs) {
        return {
    ...
    updateCaptureAccumulator(accumulator, sample, timestampMs) {
        accumulator.maxMovementNorm = Math.max(accumulator.maxMovementNorm, sample.movingAnkleLiftNorm ?? 0, sample.sameSideExtensionNorm ?? 0, sample.wristsDistanceNorm ? 1 - sample.wristsDistanceNorm : 0);
        ...
        accumulator.minTorsoTibiaParallelErrorDeg = Math.min(accumulator.minTorsoTibiaParallelErrorDeg, sample.torsoTibiaParallelErrorDeg ?? accumulator.minTorsoTibiaParallelErrorDeg);
        accumulator.maxTorsoTibiaParallelErrorDeg = Math.max(accumulator.maxTorsoTibiaParallelErrorDeg, sample.torsoTibiaParallelErrorDeg ?? accumulator.maxTorsoTibiaParallelErrorDeg);
        ...
        accumulator.minKneeOverFootOffsetNorm = Math.min(accumulator.minKneeOverFootOffsetNorm, sample.kneeOverFootOffsetNorm ?? accumulator.minKneeOverFootOffsetNorm);
        accumulator.maxKneeOverFootOffsetNorm = Math.max(accumulator.maxKneeOverFootOffsetNorm, sample.kneeOverFootOffsetNorm ?? accumulator.maxKneeOverFootOffsetNorm);
        accumulator.minDowelOverMidfootOffsetNorm = Math.min(accumulator.minDowelOverMidfootOffsetNorm, sample.dowelOverMidfootOffsetNorm ?? accumulator.minDowelOverMidfootOffsetNorm);
        accumulator.maxDowelOverMidfootOffsetNorm = Math.max(accumulator.maxDowelOverMidfootOffsetNorm, sample.dowelOverMidfootOffsetNorm ?? accumulator.maxDowelOverMidfootOffsetNorm);
        ...
        accumulator.minRearKneeTargetDistanceNorm = Math.min(accumulator.minRearKneeTargetDistanceNorm, sample.rearKneeTargetDistanceNorm ?? accumulator.minRearKneeTargetDistanceNorm);
        accumulator.maxFrontFootSagittalAlignmentError = Math.max(accumulator.maxFrontFootSagittalAlignmentError, sample.frontFootSagittalAlignmentError ?? accumulator.maxFrontFootSagittalAlignmentError);
        ...
        accumulator.bestHandPlacementClass = Math.max(accumulator.bestHandPlacementClass, sample.handPlacementClass ?? 1);
        accumulator.maxSameSideExtensionNorm = Math.max(accumulator.maxSameSideExtensionNorm, sample.sameSideExtensionNorm ?? 0);
        accumulator.minElbowKneeTouchDistanceNorm = Math.min(accumulator.minElbowKneeTouchDistanceNorm, sample.elbowKneeTouchDistanceNorm ?? accumulator.minElbowKneeTouchDistanceNorm);
    ...
    hasReturnedToStart(sample, baseline) {
        switch (this.task.patternKey) {
            case 'deepSquat':
                return (Math.abs((sample.kneeAngleDeg ?? baseline.kneeAngleDeg) - baseline.kneeAngleDeg) <= FMS_TUNING.deepSquat.returnKneeWindowDeg &&
            ...
            case 'rotaryStability':
                return (normalize((sample.movingWristX ?? 0) - (baseline.movingWristX ?? sample.movingWristX ?? 0), baseline.bodyScale) <= FMS_TUNING.rotaryStability.returnWindowNorm &&
    ...
    didMovementComplete(sample, baseline, accumulator, timestampMs) {
    ...
    scoreDeepSquat(accumulator) {
        ...
        const notes = [];
        if (!passesBase) {
            if (accumulator.maxTorsoTibiaParallelErrorDeg > 15)
                notes.push('Torso and tibia did not stay parallel enough at depth.');
            if (!femurBelowHorizontal)
                notes.push('Depth did not clearly reach the below-horizontal target.');
            if (accumulator.minLumbarProxyDeg < 145)
                notes.push('The trunk folded too much at the bottom position.');
            if (this.mode !== 'heelsElevated')
                notes.push('Retry with heels elevated on the board if score 3 is not available.');
        }
    ...
    scoreHurdleStep(accumulator) {
        const completed = accumulator.maxMovingAnkleLiftNorm >= 0.1;
        const cleanAlignment = accumulator.maxTrunkSwayDeg <= 10 &&
        ...
        const notes = [];
        if (!completed)
            notes.push('The step did not clear a strong enough lift-and-return pattern.');
        if (accumulator.maxBalanceRecoveryNorm > 0.1)
            notes.push('Balance recovery was detected during the step.');
        if (completed && !cleanAlignment)
            notes.push('The step completed, but alignment or trunk control drifted.');
    ...
    scoreInlineLunge(accumulator) {
        const completed = accumulator.minRearKneeTargetDistanceNorm <= 0.18;
        const clean = accumulator.maxTrunkSwayDeg <= 10 &&
        ...
        const notes = [];
        if (!completed)
            notes.push('The rear knee did not track close enough to the heel target.');
        if (accumulator.maxBalanceRecoveryNorm > 0.1)
            notes.push('Balance loss was detected during the lunge.');
        if (completed && !clean)
            notes.push('The lunge completed, but posture or sagittal control drifted.');
    ...
    scoreShoulderMobility(accumulator) {
        ...
        const notes = [];
        if (accumulator.maxCreepMotionNorm > 0.08)
            notes.push('The wrists appeared to creep closer after the first placement.');
        if (score === 1)
            notes.push('The wrist distance stayed wider than one and a half hand lengths.');
        return {
            score,
    ...
    scoreActiveStraightLegRaise(accumulator) {
        if (!this.lastSample) {
            return {
                score: 1,
                confidence: 0.4,
        ...
        const kneeIntegrity = accumulator.minMovingKneeAngleDeg >= 160 &&
            accumulator.minGroundedKneeAngleDeg >= 170 &&
            accumulator.maxGroundedHeelLiftNorm <= 0.03;
        const score = kneeIntegrity ? zone : Math.min(zone, 1);
        const notes = [];
        if (!kneeIntegrity)
            notes.push('The moving or grounded leg lost the straight-leg requirement.');
        if (score === 1)
            notes.push('The ankle projection stayed below the grounded knee zone.');
        return {
            score,
    ...
    scoreTrunkStabilityPushUp(accumulator) {
        const repCompleted = accumulator.maxElbowExtensionDeg >= 150;
        const cleanBodyLine = accumulator.maxBodyLineErrorDeg <= 14 &&
        ...
        if (repCompleted && cleanBodyLine) {
            if (handPlacement === 3)
                score = 3;
            else if (handPlacement >= 2)
                score = 2;
        }
        const notes = [];
        if (!repCompleted)
            notes.push('The push-up did not reach a clear rigid-body top position.');
        if (repCompleted && !cleanBodyLine)
            notes.push('The trunk rose, but the body line sagged more than the target range.');
        if (repCompleted && cleanBodyLine && handPlacement === 1)
            notes.push('The hand placement looked easier than the official score 2 line.');
        return {
            score,
    ...
    scoreRotaryStability(accumulator) {
        const sameSidePassed = this.mode === 'primary' &&
        ...
        const diagonalPassed = this.mode === 'diagonalRegression' &&
